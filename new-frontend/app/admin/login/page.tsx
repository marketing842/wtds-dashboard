...
const FEATURE_KEYS = [
  { icon: Users,     key: 'adminLogin.feature.clients' },
  { icon: KeyRound,  key: 'adminLogin.feature.credentials' },
  { icon: BarChart3, key: 'adminLogin.feature.dashboards' },
  { icon: Lock,      key: 'adminLogin.feature.secure' },
] as const
...
export default function AdminLoginPage() {
  ...
  const taglineLines = t('adminLogin.tagline').split('\n')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || data.role !== 'admin') { setError(t('adminLogin.errorInvalid')); setLoading(false); return }
      setToken(data.token)
      router.push('/admin')
    } catch {
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <div className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-full"
            ...
            <ShieldCheck className="w-3.5 h-3.5" style={{ color: COLOR }} />
          ...
          <div className="mt-6 space-y-2.5">
            {FEATURE_KEYS.map(({ icon: Icon, key }) => (
              <div key={key} className="flex items-center gap-3">
                ...
                <span style={{ fontSize: '13px', fontWeight: 500, color: 'rgba(250,250,250,0.55)' }}>{t(key)}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="w-full max-w-[380px]">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" disabled={loading}
                ...
                {loading
                  ? <><Loader2 className="w-4 h-4 animate-spin" /> {t('adminLogin.signingIn')}</>
                  : t('adminLogin.signIn')}
              </button>
            </form>
